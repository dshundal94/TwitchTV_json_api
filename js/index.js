/*Using a hardcode JSON variable because Twitch API requires an API key, and don't want to make that public. So I will use a typical JSON that is returned when making a call to the API*/
...
for (var i = 0; i < json_example.length; i++) {
 var status;
 if (json_example[i].stream === null) {
   status = 'offline';
 } else if (json_example[i].stream === undefined) {
   status = 'DNE';
 } else {
   status = 'online';
 }
  ...
  var url;
  if (status === 'online'){
    picture = json_example[i].stream.logo;
    name = json_example[i].stream.display_name;
    description = json_example[i].stream.status;
    trimmed = description.substring(0, 40) + "...";
    url = json_example[i].stream.url;
  } else if (status === 'offline') {
    picture = 'https://i.ytimg.com/vi/0SoDhELNoBA/maxresdefault.jpg';
    name = json_example[i].display_name;
    trimmed = "Offline";
    url = "https://www.twitch.tv/" + name;
  } else {
    name = 'Does Not Exist';
    trimmed = "Not a valid account";
    url = "#";
  }
  html = '<div class="row ' + 
          status + '"><div class="col-xs-2 col-sm-1" id="icon"><img src="' + 
          picture + '" class="logo"></div><div class="col-xs-10 col-sm-3" id="name"><a href="' + 
          url + '" target="_blank">' + 
          name + '</a></div><div class="col-xs-10 col-sm-8" id="streaming">'+  
          trimmed + '</span></div></div>'
$("#display").append(html);
}